...
router.get('/', (req, res) => {
  db.query('SELECT cod_produto, nome_produto, estoque FROM Produtos ORDER BY nome_produto', (err, resultados) => {
...
router.post('/novo', (req, res) => {
  ...
  db.query('INSERT INTO Produtos (cod_produto, nome_produto, categoria, subcategoria, preco_real, disponibilidade, estoque) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [cod_produto, nome_produto, categoria, subcategoria, preco_real || 0, 'Pronta Entrega', estoque || 0],
    (err) => {
...
router.post('/editar/:id', (req, res) => {
  ...
  db.query('UPDATE Produtos SET nome_produto=?, categoria=?, subcategoria=?, preco_real=?, estoque=?, disponibilidade=? WHERE cod_produto=?',
    [nome_produto, categoria, subcategoria, preco_real || 0, estoque || 0, disponibilidade || 'Pronta Entrega', id],
    (err) => {
...
router.post('/deletar/:id', (req, res) => {
  const id = req.params.id;
  db.query('DELETE FROM Produtos WHERE cod_produto = ?', [id], (err) => {
